...
export default class PlayBack extends BasicTask {
  constructor() {
    super(PlayTask.PLAY_BACK)
  }
  public run() {
    // 任务状态置为Running
    ...
    // 播放上一曲的准备工作（给_currMusicIndex赋予新值）
    if (ActionDataModel.I.playPattern === PlayPattern.LIST_REPEAT) {
      // 如果是列表循环下的“上一曲”，就让currIndex减一
      ActionDataModel.I.reduceCurrMusicIndex();
    } else if (ActionDataModel.I.playPattern === PlayPattern.RANDOM_PLAY) {
      // 随机循环就优先取_randomMusicList里的，不行就随机上一曲
    ...
    showToast(`上一曲${ActionDataModel.I.currMusicName ? ': ' + ActionDataModel.I.currMusicName : ''}`);
